...
class Piece():
    ...
    def move(self, up_end_index):
        ...
        match self.piece_type:
            case Chess.PAWN:
                self.pawn_move(up_end_index)
                self.move_finalize(up_end_index)
            case Chess.ROOK:
                self.rook_move(up_end_index)
                self.move_finalize(up_end_index)
            case Chess.BISHOP:
                self.bishop_move(up_end_index)
                self.move_finalize(up_end_index)
            case Chess.HORSE:
                self.horse_move(up_end_index)
                self.move_finalize(up_end_index)
            case Chess.QUEEN:
                self.rook_move(up_end_index)
                self.bishop_move(up_end_index)
                self.move_finalize(up_end_index)
            case Chess.KING:
                self.king_move(up_end_index)
                self.move_finalize(up_end_index)


    def pawn_move(self, up_end_index):
    ...
    def rook_move(self, up_end_index):
        if self.has_moved == False:
            match self.index:
                case 0:
    ...
    def bishop_move(self, up_end_index):
        free_lu = True
        free_ru = True
        free_ld = True
        free_rd = True
